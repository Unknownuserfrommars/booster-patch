...
NodeStatus StrikerDecide::tick() {
    ...
    )
    {
        if (brain->data->kickType == "cross") newDecision = "cross";
            ...
            // Detect opponent possession: ball in our half and we are not lead attacker
            bool oppHasBall = ball.posToField.x < brain->config->fieldDimensions.length / 2 - 2.0
                && !brain->data->tmImLead
                && ballRange < defensiveStealRange * 1.5;
