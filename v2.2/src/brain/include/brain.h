...
public:
    ...
    // 当前根据 GameController 判定在场的我方 playerId
    vector<int> getOnCourtPlayerIds(bool includeSelf = false);

    // 当前在场且通讯仍然在线的队友 playerId
    vector<int> getReachableTeammateIds();

    // 发送队友通信 CID. targetPid 通过 CID 参数编码, 网络层仍发给所有已发现队友.
    bool sendTeamSignal(int cid, bool force = false);

    // 获取某个 CID 的发送倾向参数, 0.0=不主动发, 1.0=总是允许.
    double getTeamSignalTendency(int cid);
...
private:
    ...
    bool shouldSendTeamSignal(int cid, bool force = false);
    bool queueTeamCommand(int cmd);
    void handleReceivedTeamSignal(int cid, int senderId);
