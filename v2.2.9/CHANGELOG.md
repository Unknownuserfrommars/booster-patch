# Patch v2.2.9 — CHANGELOG

## v2.2.9 — Fix v2.2.8 Field Regressions (2026-07-14)

Three bugs observed running v2.2.8 on the robot, all traced to the aggressive speed push and the
GlanceAtGoal node. Config/XML only — no C++ change, no recompile.

### Bug: robot stops midway / stops chasing completely
- Root cause: v2.2.8 Chase `vx_limit` 1.8 outran the head's ball tracking (`CamTrackBall`) — the ball
  left frame, `ball_location_known` went false after 3 s, Chase hard-stopped and the decision flipped to
  `find`, whose FindBall subtree scans with the head while the body stands still (looked like "stopped
  chasing"). The `avoid_during_chase` 0.5 m/s veer + sigmoid stop-to-turn caused the "stops midway".
- Fix: Chase `vx_limit` 1.8 → **1.2** (still faster than the original 0.9, but the head keeps the ball),
  `vtheta_limit` 2.0 → 1.5, and `obstacle_avoidance.chase_ao_safe_dist` 1.5 → **1.0** (avoidance only
  triggers when genuinely close, cutting false stalls from depth misdetections).

### Bug: not shooting in the correct direction
- Root cause: v2.2.8 `near_ball_speed_limit` 1.0 meant the robot reached the ball still moving fast and
  kicked before settling, scattering the aim.
- Fix: `near_ball_speed_limit` 1.0 → **0.6** (settle before the kick). The kick-direction geometry
  (`calcKickApproachAngle`, walks through the ball along `kickDir`) was verified correct and is unchanged.

### Bug: GlanceAtGoal is bad
- Root cause: the node commands `setVelocity(0,0,0)` — a 0.6–0.9 s dead freeze mid-attack — and swings the
  head off the ball; in a live match that just loses the ball / invites a steal, and the goalposts often
  aren't actually reacquired so it burns the full window for nothing.
- Fix: `strategy.glance_at_goal.enable` true → **false**. Aim now relies on normal localization + goalpost
  detection during ball tracking. (Node/code left in place, dormant, for later rework.)

### Retained from v2.2.8
- `soft_kickoff: true` (gentle short kick-off) is kept — that one worked as intended.

> Load: config/XML only — `./scripts/build_brain.sh` (`--symlink-install`) then `start.sh`.

---

## v2.2.8 — Chase at Max Speed + Soft Kick-off (2026-07-11)

### Chase speed / mid-chase stalling
- `subtree_striker_play.xml` Chase node: `vx_limit` 0.9 → **1.8** (= `robot.vx_limit`, the `setVelocity`
  hard ceiling — the robot's true max forward speed; chase was running at half). `vtheta_limit` 1.0 → 2.0
  so the robot turns faster and spends less time in Chase's `vx *= sigmoid(|targetDir|,1,3)` "stop-to-turn"
  low-speed phase — a major cause of the visible mid-chase halting.
- `strategy.near_ball_speed_limit` 0.6 → 1.0: less crawling within 2 m of the ball. (Note: `setVelocity`
  has a 0.3 m/s floor; dial back toward 0.6–0.8 if fast approach starts bumping the ball before the kick.)
- Remaining stall causes documented but left as opt-in toggles: `obstacle_avoidance.avoid_during_chase`
  (drops to a fixed 0.5 m/s veer near obstacles/misdetections — likely #1 on a crowded field) and the
  `ball_location_known` dead-stop after >3 s without the ball.

### Soft kick-off
- `strategy.soft_kickoff` false → **true**: during the kickoff window (`isKickingOff` /
  `isFreekickKickingOff`) the Kick node uses `soft_kickoff_speed` (0.3 m/s) instead of 0.9–1.2, so the
  kick-off is a gentle short push (~⅓ the momentum) for controlled possession instead of a long blast.
  Applies to both game and freekick kickoffs. `soft_kickoff_speed` 0.3 is effectively the softest
  possible (0.3 m/s `setVelocity` floor); for an even shorter kick-off, shorten kick contact time
  (`min_msec_kick`) rather than the speed.

> Stability note: 1.8 m/s + 2.0 rad/s turns is aggressive for a K1 biped. If it falls during chase,
> lower Chase `vx_limit` toward ~1.4. All changes are config/XML only — run `./scripts/build_brain.sh`
> (`--symlink-install`, fast, no C++ recompile) then `start.sh` to load them.
