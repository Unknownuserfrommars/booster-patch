...
def bootstrap_ros_environment():
    ...
    needs_booster_msgs = has_flag('--discover') or has_flag('--api-id')
    needs_booster_interface = has_flag('--api-id')
...
SYSTEM_VISION_CONFIG = '/opt/booster/vision.yaml'
CALIBRATION_QUALITY_SCALE = (
    'Outstanding <1.5 px · Excellent 1.5~3 px · Good 3~5 px · '
    'Average 5~8 px · Poor 8~15 px · Critical >15 px'
)
...
class AutoHandEyeCalib(Node):
    def __init__(self, config_path, mode='auto', api_id=None,
                 ...
                 compressed_topic=None, prompt_save=True,
                 save_input_config=False, save_system_config=False):
        ...
        self.prompt_save = prompt_save
        self.save_input_config = save_input_config
        self.save_system_config = save_system_config
    ...
    def _ask_yes_no(self, prompt):
        if not self.prompt_save or not sys.stdin.isatty():
            return False
        try:
    ...
    def _copy_file_if_exists(self, src_path, backup_path):
    ...
    def _save_to_input_config(self, result, log_dir, ts):
        if not self.config_path:
            self.get_logger().warn('No --config path was provided; cannot overwrite input config.')
            return False

        backup_path = os.path.join(log_dir, f'{os.path.basename(self.config_path)}.input_backup_{ts}')
        try:
            if self._copy_file_if_exists(self.config_path, backup_path):
                self.get_logger().info(f'Backed up input config: {backup_path}')
            self._write_yaml(self.config_path, result)
        except OSError as e:
            self.get_logger().error(f'Failed to overwrite input config {self.config_path}: {e}')
            return False

        self.get_logger().info(f'Overwrote input config: {self.config_path}')
        return True

    def _save_to_system_config(self, result, ts):
        target = SYSTEM_VISION_CONFIG
        try:
            os.makedirs(os.path.dirname(target), exist_ok=True)
            self._write_yaml(target, result)
        except PermissionError:
            tmp_path = f'/tmp/vision.yaml.auto_handeye_{ts}'
            try:
                self._write_yaml(tmp_path, result)
                proc = subprocess.run(['sudo', 'mv', tmp_path, target], check=False)
            except OSError as e:
                self.get_logger().error(f'Failed to save temporary system config: {e}')
                return False
            if proc.returncode != 0:
                self.get_logger().error(f'sudo mv failed; result remains at {tmp_path}')
                return False
        except OSError as e:
            self.get_logger().error(f'Failed to save {target}: {e}')
            return False

        self.get_logger().info(f'Saved system config: {target}')
        return True

    def _quality_for_error(self, error):
        if not np.isfinite(error) or error > 15.0:
            return 'Critical'
        if error < 1.5:
            return 'Outstanding'
        if error < 3.0:
    ...
    def _format_error(self, error):
        return f'{error:.7f}' if np.isfinite(error) else 'nan'

    def _prompt_for_result_saves(self, result, log_dir, ts, error, quality):
        error_text = self._format_error(error)
        save_input = self.save_input_config or self._ask_yes_no(
            f'Store THIS calibration as input vision.yaml ({self.config_path or "<none>"})? '
            f'error={error_text} px, quality={quality}'
        )
        if save_input:
            self._save_to_input_config(result, log_dir, ts)
        else:
            self.get_logger().info('Input config not overwritten.')

        save_system = self.save_system_config or self._ask_yes_no(
            f'Store THIS calibration as {SYSTEM_VISION_CONFIG}? '
            f'error={error_text} px, quality={quality}'
        )
        if save_system:
            self._save_to_system_config(result, ts)
        else:
            self.get_logger().info(f'{SYSTEM_VISION_CONFIG} not overwritten.')
    ...
    def _move_head(self, pitch_deg, yaw_deg):
        ...
        self._publish_loco_api(
            self.head_api_id,
            {'pitch': float(pitch_rad), 'yaw': float(yaw_rad)}
        )
    ...
    def _publish_loco_api(self, api_id, body):
        if self.head_pub is None:
            return False
        msg = RpcReqMsg()
        msg.uuid = str(uuid.uuid4())
        msg.header = json.dumps({'api_id': int(api_id)})
        msg.body = json.dumps(body) if isinstance(body, dict) else str(body)
    ...
    def _restore_prepare_mode(self):
        if self.mode_switched_for_auto and rclpy.ok():
            self._publish_loco_api(LOCO_API_CHANGE_MODE, {'mode': ROBOT_MODE_PREPARE})
            self.mode_switched_for_auto = False
    ...
    def _run_calibration(self):
        ...
        # Reprojection errors:
        # - PnP error checks each individual chessboard detection.
        # - Hand-eye error checks whether the solved camera-to-head transform
        #   explains all captures using one fixed board pose in the base frame.
        total_pnp_err = 0.0
        total_handeye_err = 0.0
        n_pnp = 0
        n_handeye = 0
        if self.intrinsics_received:
            for f in self.captured_frames:
                ...
                total_pnp_err += cv2.norm(cn, pts_proj, cv2.NORM_L2) / len(pts_proj)
                n_pnp += 1

            T_eye2head = rt_to_4x4(R_c2g, t_c2g)
            first = self.captured_frames[0]
            R_board0, _ = cv2.Rodrigues(first['board_rvec'])
            T_board2eye0 = rt_to_4x4(R_board0, first['board_tvec'])
            T_head2base0 = rt_to_4x4(first['head_R'], first['head_t'])
            T_board2base = T_head2base0 @ T_eye2head @ T_board2eye0

            for f in self.captured_frames:
                T_head2base = rt_to_4x4(f['head_R'], f['head_t'])
                T_board2eye_pred = np.linalg.inv(T_head2base @ T_eye2head) @ T_board2base
                rvec_pred, _ = cv2.Rodrigues(T_board2eye_pred[:3, :3])
                tvec_pred = T_board2eye_pred[:3, 3:4]
                pts_proj, _ = cv2.projectPoints(
                    self.board_points_3d, rvec_pred, tvec_pred,
                    self.camera_matrix, self.dist_coeffs
                )
                cn = np.array(f['corners'], dtype=np.float32).reshape(-1, 1, 2)
                total_handeye_err += cv2.norm(cn, pts_proj, cv2.NORM_L2) / len(pts_proj)
                n_handeye += 1

        mean_pnp_reproj = total_pnp_err / n_pnp if n_pnp > 0 else float('nan')
        mean_handeye_reproj = total_handeye_err / n_handeye if n_handeye > 0 else float('nan')
        ...
        quality = self._quality_for_error(mean_handeye_reproj)
        error_text = self._format_error(mean_handeye_reproj)
        ...
        self.get_logger().info(f'Calibration error scale: {CALIBRATION_QUALITY_SCALE}')
        self.get_logger().info(f'Exact calibration error: {error_text} px -> {quality}')
        self.get_logger().info(f'Mean PnP reprojection error: {self._format_error(mean_pnp_reproj)} px')
        ...
        handeye_cfg['reprojection_error'] = float(mean_handeye_reproj)
        handeye_cfg['pnp_reprojection_error'] = float(mean_pnp_reproj)
        handeye_cfg['quality'] = quality
        handeye_cfg['quality_scale'] = CALIBRATION_QUALITY_SCALE
        ...
        self._write_yaml(result_path, result)

        self.get_logger().info(f'Saved: {result_path}')
        self._restore_prepare_mode()
        self._prompt_for_result_saves(result, log_dir, ts, mean_handeye_reproj, quality)
        self.get_logger().info('=' * 60)
...
def main():
    ...
    parser.add_argument('--no-save-prompt', action='store_true',
                        help='Do not ask y/n questions after calibration; only save timestamped result')
    parser.add_argument('--save-input-config', action='store_true',
                        help='After calibration, overwrite --config without asking')
    parser.add_argument('--save-system-config', action='store_true',
                        help=f'After calibration, save to {SYSTEM_VISION_CONFIG} without asking')
    ...
    node = AutoHandEyeCalib(
        ...
        prompt_save=not args.no_save_prompt,
        save_input_config=args.save_input_config,
        save_system_config=args.save_system_config,
    )
