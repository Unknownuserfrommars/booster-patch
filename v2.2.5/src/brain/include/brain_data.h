...
public:
    ...
    int8_t kickSubType = 0; // 0=normal, 1=freekick, 2=quickshot, 3=deflection, 4=clearance
    int8_t kickLeg = 0; // 0=right, 1=left
    bool isClearance = false;
    bool isOpponentPossession = false;
    int ballState = 0; // 0=our, 1=opp, 2=contested, 3=free, 4=danger
    int oppPossessionFrames = 0;
    double prevBallX = 0.0, prevBallY = 0.0;
    bool hasPrevBall = false;
    rclcpp::Time prevBallTime = rclcpp::Time(0, 0, RCL_ROS_TIME);
    ...
    int tmReceivedCmdFromId = -1; // 最近收到指令的发送方 playerId
    int tmLastProcessedCmdId[HL_MAX_NUM_PLAYERS] = {0}; // 每个队友最后处理过的 cmdId, 避免不同队友 cmdId 冲突
    std::deque<std::pair<int, int>> tmPendingCommands; // {cmd, cmdId}, unicast 线程逐条发出
    std::mutex tmPendingCommandsMutex;
    std::deque<std::tuple<int, int, int>> tmReceivedCmdQueue; // {cmd, senderId, senderCmdId}
    std::mutex tmReceivedCmdMutex;
    ...
    rclcpp::Time tmLastSignalSendTime; // 最后一次发送 team signal 的时间
    int tmLastSignalSent = 0; // 最近发送的 CID
    int tmDangerLevel = 0; // 0-9, 队友同步的危险等级
    rclcpp::Time tmDangerTime;
    string tmDefensiveMode = ""; // "press" | "retreat" | "hold" | ""
    rclcpp::Time tmDefensiveModeTime;
    string tmFormationAdjust = ""; // "spread" | "compact" | ""
    rclcpp::Time tmFormationAdjustTime;
    bool tmPassRequested = false;
    int tmPassTargetId = -1;
    bool tmExpectingPass = false;
    int tmPassFromId = -1;
    rclcpp::Time tmPassSignalTime;
    bool tmCrossIncoming = false;
    rclcpp::Time tmCrossSignalTime;
    bool tmClearRequested = false;
    rclcpp::Time tmClearRequestTime;
    bool tmBallLostBy[HL_MAX_NUM_PLAYERS] = {false};
    bool tmFallenPlayers[HL_MAX_NUM_PLAYERS] = {false};
    bool tmStuckPlayers[HL_MAX_NUM_PLAYERS] = {false};
    bool tmImInVisualKick = false; // 自己是否处于 visual kick 模式
    bool shouldExitRLVisionKick = false; // 是否需要主动退出 visual kick 模式

    // 3v3 conservative 2-on-1 wall-pass tactic.
    TwoToOneRole twoToOneRole = TwoToOneRole::INACTIVE;
    TwoToOneStage twoToOneStage = TwoToOneStage::IDLE;
    int twoToOneCarrierId = -1;
    int twoToOneSupportId = -1;
    int twoToOneFlankSign = 0; // +1 = field-left lane, -1 = field-right lane
    int twoToOneOpponentCount = 0;
    bool twoToOneSupportReady = false;
    bool twoToOneRunnerReady = false;
    bool twoToOneStartSignalSent = false;
    bool twoToOneReadySignalSent = false;
    bool twoToOneRunReadySignalSent = false;
    bool twoToOneFirstPassSignalSent = false;
    bool twoToOneReturnPassSignalSent = false;
    Point2D twoToOneOpponentPos{0.0, 0.0};
    Pose2D twoToOneSupportTarget{0.0, 0.0, 0.0};
    Pose2D twoToOneRunTarget{0.0, 0.0, 0.0};
    rclcpp::Time twoToOneStartTime;
    rclcpp::Time twoToOneStageTime;
    rclcpp::Time twoToOneLastSignalTime;
    ...
    rclcpp::Time lastGoalObservationTime; // 上次看到对方半场门柱的时间, 用于射门前判断瞄准信息是否新鲜
