...
void resetTwoToOneTactic(Brain *brain) {
    ...
    brain->data->twoToOneRole = TwoToOneRole::NONE;
...
bool isTwoToOneTacticTimedOut(Brain *brain) {
    ...
    return brain->data->twoToOneRole != TwoToOneRole::NONE &&
...
bool isTwoToOneAbortNeeded(Brain *brain) {
    if (brain->data->twoToOneRole == TwoToOneRole::NONE) return false;
...
void abortTwoToOneTactic(Brain *brain, const string &reason) {
    if (brain->data->twoToOneRole != TwoToOneRole::NONE) {
...
NodeStatus StrikerDecide::tick() {
    ...
    if (twoToOneEnabled && brain->data->twoToOneRole != TwoToOneRole::NONE) {
        if (isTwoToOneAbortNeeded(brain)) {
    ...
    )
    {
        if (brain->data->kickType == "cross") newDecision = "cross";
            ...
            for (auto& obs : brain->data->getObstacles()) {
                if (obs.label == "Goalpost" || obs.label == "LCross" || obs.label == "TCross" || 
                    obs.label == "XCross" || obs.label == "PenaltyPoint" || obs.label == "BRMarker") continue;
                double dx = obs.posToField.x - ball.posToField.x;
                double dy = obs.posToField.y - ball.posToField.y;
                double d = sqrt(dx*dx + dy*dy);
                if (d < oppNearestDist) oppNearestDist = d;
            }
            if (oppNearestDist > 10.0) oppNearestDist = 10.0;
            auto now = brain->get_clock()->now();
            ...
            if (enableStrikerDefense) {
                if (oppHasBall && ballRange < defensiveStealRange) {
                    ...
                    double riskRoll = (double)rand() / RAND_MAX;
                    if (riskRoll < defensiveRiskTolerance) {
                        newDecision = "steal";
                        brain->data->isClearance = true;
                        brain->data->kickSubType = 4;
                    } else {
            ...
            int signalToSend = TeamSignal::NONE;
            if (dangerActive) {
            ...
            if (signalToSend != TeamSignal::NONE) {
