    ...
    for (auto it = _teammate_addresses.begin(); it != _teammate_addresses.end();) {
        ...
        if (timeDiff > TEAMMATE_TIMEOUT_MS * 1e6) {
            cout << YELLOW_CODE << format("Teammate id %d timed out", it->second.playerId) 
                 << RESET_CODE << endl;
            int tmIdx = it->second.playerId - 1;
            if (tmIdx >= 0 && tmIdx < HL_MAX_NUM_PLAYERS) {
                brain->data->tmLastProcessedCmdId[tmIdx] = 0;
            }
...
void BrainCommunication::unicastCommunication() {
    ...
    while (_unicast_communication_flag) {
        ...
        {
            std::lock_guard<std::mutex> pendingLock(brain->data->tmPendingCommandsMutex);
            if (!brain->data->tmPendingCommands.empty()) {
                auto pending = brain->data->tmPendingCommands.front();
                brain->data->tmPendingCommands.pop_front();
                brain->data->tmMyCmd = pending.first;
                brain->data->tmMyCmdId = pending.second;
            }
        }
...
void BrainCommunication::spinCommunicationReceiver() {
    ...
    while (_receive_communication_flag) {
        ...
        // 检查是否收到了该队友的新指令. cmdId 是每个发送者自己的序号, 不能作为全队唯一序号.
        if (msg.cmdId > brain->data->tmLastProcessedCmdId[tmIdx]) {
            brain->data->tmLastProcessedCmdId[tmIdx] = msg.cmdId;
            if (msg.cmdId > brain->data->tmCmdId) {
                brain->data->tmCmdId = msg.cmdId;
            }
            brain->data->tmReceivedCmd = msg.cmd;
            brain->data->tmReceivedCmdFromId = msg.playerId;
            {
                std::lock_guard<std::mutex> cmdLock(brain->data->tmReceivedCmdMutex);
                if (brain->data->tmReceivedCmdQueue.size() >= 16) {
                    brain->data->tmReceivedCmdQueue.pop_front();
                }
                brain->data->tmReceivedCmdQueue.push_back({msg.cmd, msg.playerId, msg.cmdId});
            }
