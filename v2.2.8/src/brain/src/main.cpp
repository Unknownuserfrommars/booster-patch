...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        std::cerr << "[FATAL] brain init: " << e.what() << std::endl;
        rclcpp::shutdown();
        return 1;
    } catch (...) {
        std::cerr << "[FATAL] brain init: unknown" << std::endl;
        rclcpp::shutdown();
        return 1;
    }
    ...
    thread t([&brain]() {
        while (rclcpp::ok()) {
            try {
                ...
                RCLCPP_FATAL(brain->get_logger(), "TICK: %s", e.what());
                std::cerr << "[FATAL] tick: " << e.what() << std::endl;
                rclcpp::shutdown();
                break;
            } catch (...) {
                std::cerr << "[FATAL] tick: unknown" << std::endl;
                rclcpp::shutdown();
                break;
            }
        } 
    ...
    thread t1([&brain, &argc, &argv]() {
        try {
            ...
            std::cerr << "[FATAL] callback: " << e.what() << std::endl;
            rclcpp::shutdown();
        } catch (...) {
            std::cerr << "[FATAL] callback: unknown" << std::endl;
            rclcpp::shutdown();
        }
    });
