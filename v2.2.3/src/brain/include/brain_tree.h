...
private:
    ...
    rclcpp::Time _kickCommitTime;   // 进入踢球类决策的时刻, 用于短暂锁定决策防止抖动
    rclcpp::Time _lastGlanceTime;   // 上次 glance 的时刻, 用于冷却
    rclcpp::Time _glanceStartTime;  // 本次 glance 开始的时刻, 用于保持决策直到观察完成
    double _lastRiskRoll = 0.0;     // 防守 steal/defend 的骰子结果缓存, 防止每 tick 重掷导致决策抖动
    rclcpp::Time _lastRiskRollTime;
};
...
// 射门前先瞄一眼球门方向: 主动看向对方球门以检测门柱/重定位, 防止靠过期定位盲踢 (P1-1)
class GlanceAtGoal : public StatefulActionNode
{
public:
    GlanceAtGoal(const string &name, const NodeConfig &config, Brain *_brain) : StatefulActionNode(name, config), brain(_brain) {}
    static PortsList providedPorts() { return {}; }
    NodeStatus onStart() override;
    NodeStatus onRunning() override;
    void onHalted() override {};
...
public:
    ...
    // 重入冷却: 距上次退出 VisualKick 是否已超过 minIntervalMsec (band split 防抖用)
    static bool isMinIntervalSatisfied(double minIntervalMsec, const rclcpp::Time &now);
...
private:
    ...
    // Stall 交接: 球已在脚下却长时间没被踢走时, 退出 RL 模式交给脚本 Kick
    bool _hasStallAnchor = false;
    Point2D _stallAnchorPos{0.0, 0.0};
    rclcpp::Time _stallAnchorTime;
...
class TwoToOneSupport : public SyncActionNode
{
public:
    TwoToOneSupport(const std::string &name, const NodeConfig &config, Brain *_brain) : SyncActionNode(name, config), brain(_brain) {}
    static BT::PortsList providedPorts() {
        return {
            InputPort<double>("dist_tolerance", 0.55, ""), InputPort<double>("theta_tolerance", 0.35, ""),
            InputPort<double>("vx_limit", 0.8, ""), InputPort<double>("vy_limit", 0.6, ""),
            InputPort<double>("vtheta_limit", 1.2, ""),
        };
    }
    BT::NodeStatus tick() override;
private:
...
class TwoToOneRun : public SyncActionNode
{
public:
    TwoToOneRun(const std::string &name, const NodeConfig &config, Brain *_brain) : SyncActionNode(name, config), brain(_brain) {}
    static BT::PortsList providedPorts() {
        return {
            InputPort<double>("dist_tolerance", 0.55, ""), InputPort<double>("theta_tolerance", 0.45, ""),
            InputPort<double>("vx_limit", 0.9, ""), InputPort<double>("vy_limit", 0.6, ""),
            InputPort<double>("vtheta_limit", 1.4, ""),
        };
    }
    BT::NodeStatus tick() override;
private:
