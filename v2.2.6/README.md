...
Current patch: **v2.2.6** — tournament anti-interference rules compliance: the official `kill_booster_server.sh` (disables phone App/Bluetooth connectivity during matches) is bundled and wired into `start.sh`. Includes all v2.2.5 fixes (critical ABORT signal fix, race config profile, vision calibration hotfix).
