...
NodeStatus StrikerDecide::tick() {
    ...
    )
    {
        if (brain->data->kickType == "cross") newDecision = "cross";
            ...
            // --- v1.2.1: 5-state possession detection ---
            ...
            int hysterFrames = (int)(hysterSecs * 30.0); // ~30fps estimate

            // Nearest opponent to ball (estimate via obstacles + ball position)
            ...
            if (oppNearestDist > 10.0) oppNearestDist = 10.0; // no obstacles

            // Ball velocity (simple delta from previous position)
            double ballVx = (ball.posToField.x - brain->data->prevBallX) * 30.0;
            double ballVy = (ball.posToField.y - brain->data->prevBallY) * 30.0;
            ...
            // Determine raw state (before hysteresis)
            int rawState = 0; // 0=our, 1=opp, 2=contested, 3=free
            double ourDist = ballRange;

            if (ourDist < attackProtDist && ballMovingToTheirGoal) {
                rawState = 0; // OUR_POSSESSION - attacker is on it
            } else if (oppNearestDist + possMargin < ourDist && ballMovingToOurGoal) {
                rawState = 1; // OPP_POSSESSION - opponent clearly has it
            } else if (ourDist < defensiveStealRange * 2.0 && oppNearestDist < defensiveStealRange * 2.0) {
                rawState = 2; // CONTESTED - both teams near
            } else {
                rawState = 3; // FREE_BALL - nobody close
            }

            // Hysteresis: state must persist for N frames
            ...
            if (enableQuickShot && ballRange < quickshotMaxRange && ballX > 0.0) {
                newDecision = "quickshot";
                brain->data->kickSubType = 2;
            } else if (deflectionWeight > 0.0 && !angleGoodForKick && ballRange < KICK_RANGE * 1.2 && brain->distToObstacle(dir_rb_f) < 1.5) {
                double roll = (double)rand() / RAND_MAX;
                if (roll < deflectionWeight) {
                    newDecision = "kick";
                    brain->data->kickSubType = 3;
                } else {
                    newDecision = "adjust";
                    brain->data->kickSubType = 0;
                }
            } else if (enableStrikerDefense && oppHasBall && ballRange < defensiveStealRange) {
                newDecision = "steal";
                brain->data->kickSubType = 4;
                brain->data->isClearance = true;
            } else if (enableStrikerDefense && oppHasBall && ballRange < defensiveStealRange * 2.0) {
            ...
            } else if (enableStrikerDefense && oppHasBall) {
                newDecision = "defend";
                brain->data->kickSubType = 0;
