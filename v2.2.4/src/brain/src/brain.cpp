...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<double>("strategy.kick_range", 0.65);
    declare_parameter<double>("strategy.kick_theta_range", 0.35);
    ...
    declare_parameter<double>("strategy.auto_visual_kick_enable_dist_min", 1.2);
    ...
    declare_parameter<double>("strategy.auto_visual_kick_handoff_range", 0.85);
    declare_parameter<int>("strategy.auto_visual_kick_stall_ms", 2500);
    declare_parameter<int>("strategy.auto_visual_kick_reentry_cooldown_ms", 4000);
    ...
    declare_parameter<bool>("strategy.two_to_one.enabled", false);
    declare_parameter<double>("strategy.two_to_one.execution_distance_threshold", 5.0);
    declare_parameter<double>("strategy.two_to_one.forward_corridor_width", 2.0);
    declare_parameter<double>("strategy.two_to_one.support_flank_x_offset", 0.8);
    declare_parameter<double>("strategy.two_to_one.support_flank_y_offset", 1.2);
    declare_parameter<double>("strategy.two_to_one.runner_x_offset", 1.4);
    declare_parameter<double>("strategy.two_to_one.runner_y_offset", 0.45);
    declare_parameter<double>("strategy.two_to_one.ready_radius", 0.55);
    declare_parameter<double>("strategy.two_to_one.receive_ball_range", 1.0);
    declare_parameter<int>("strategy.two_to_one.max_setup_time_ms", 2500);
    declare_parameter<int>("strategy.two_to_one.max_total_time_ms", 7000);
    declare_parameter<int>("strategy.two_to_one.comm_timeout_ms", 1400);
    declare_parameter<int>("strategy.two_to_one.first_pass_kick_ms", 900);
    declare_parameter<int>("strategy.two_to_one.return_pass_kick_ms", 900);
    declare_parameter<double>("strategy.two_to_one.min_ball_confidence", 30.0);
    declare_parameter<bool>("strategy.two_to_one.abort_extra_opponents", true);
    declare_parameter<bool>("strategy.glance_at_goal.enable", true);
    declare_parameter<int>("strategy.glance_at_goal.msecs", 600);
    declare_parameter<int>("strategy.glance_at_goal.max_aim_age_ms", 4000);
    declare_parameter<int>("strategy.glance_at_goal.cooldown_ms", 5000);
    declare_parameter<bool>("strategy.team_signals.enabled", true);
    declare_parameter<int>("strategy.team_signals.cooldown_ms", 1500);
    declare_parameter<int>("strategy.team_signals.ttl_ms", 2500);
    declare_parameter<double>("strategy.team_signals.tendency.cid_0000", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_1000", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_1100", 0.7);
    declare_parameter<double>("strategy.team_signals.tendency.cid_1200", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_1300", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2000", 0.4);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2100", 0.5);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2200", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2300", 0.4);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2400", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2500", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2600", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2700", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2800", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_2900", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3000", 0.8);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3100", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3200", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3300", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3400", 0.5);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3401", 0.9);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3500", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_3600", 0.7);
    declare_parameter<double>("strategy.team_signals.tendency.cid_4000", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_4100", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_4200", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_4300", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_5000", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_5100", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_5200", 0.2);
    declare_parameter<double>("strategy.team_signals.tendency.cid_5201", 0.2);
    declare_parameter<double>("strategy.team_signals.tendency.cid_5300", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_5400", 0.3);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6000", 0.8);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6001", 0.8);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6100", 0.8);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6101", 0.8);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6200", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6201", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6300", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_6301", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_7000", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_7100", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_7200", 0.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_8000", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_8100", 1.0);
    declare_parameter<double>("strategy.team_signals.tendency.cid_8200", 0.9);
    declare_parameter<double>("strategy.team_signals.tendency.cid_8300", 0.0);
    ...
    declare_parameter("strategy.enable_freekick_powershot", false);
    declare_parameter("strategy.freekick_powershot_power", 9.0);
    declare_parameter("strategy.freekick_powershot_min_dist", 3.0);
    declare_parameter("strategy.enable_quickshot", false);
    declare_parameter("strategy.quickshot_max_range", 1.2);
    declare_parameter("strategy.deflection_shot_weight", 0.0);
    declare_parameter("strategy.deflection_power_boost", 2.5);
    declare_parameter("strategy.enable_freekick_deflection", false);
    declare_parameter("strategy.enable_striker_defense", false);
    declare_parameter("strategy.defensive_risk_tolerance", 0.5);
    declare_parameter("strategy.defensive_steal_range", 1.5);
    declare_parameter("strategy.defensive_clear_power", 6.0);
...
void Brain::init()
{
    ...
    data->lastGoalObservationTime = rclcpp::Time(0, 0, RCL_ROS_TIME); // 开局视为"没看过球门", 首次射门前会先 glance
    ...
    data->tmLastSignalSendTime = now;
    data->tmDangerTime = now;
    data->tmDefensiveModeTime = now;
    data->tmFormationAdjustTime = now;
    data->tmPassSignalTime = now;
    data->tmCrossSignalTime = now;
    data->tmClearRequestTime = now;
    data->twoToOneStartTime = now;
    data->twoToOneStageTime = now;
    data->twoToOneLastSignalTime = now;
...
void Brain::tick()
{
    ...
    bool setPlayStopped = gameSubStateType == "FREE_KICK" && (gameSubState == "STOP" || gameSubState == "SET");
    if (gameState == "SET" || gameState == "END" || isUnderPenalty || setPlayStopped) {
...
void Brain::pubKickMsg() {
    ...
    if (data->isClearance) {
        kickMsg.dir = toPInPI(calcClearDir() - data->robotPoseToField.theta);
    } else {
    ...
    dist = std::abs(dist);
    double power = 0.0;
    double freekickPower, deflectionBoost;
    bool enableFreeKickPower;
    double deflectionWeight;
    get_parameter("strategy.enable_freekick_powershot", enableFreeKickPower);
    get_parameter("strategy.freekick_powershot_power", freekickPower);
    get_parameter("strategy.deflection_shot_weight", deflectionWeight);
    get_parameter("strategy.deflection_power_boost", deflectionBoost);

    if (data->kickSubType == 1 && enableFreeKickPower) {
        power = freekickPower;
    } else if (data->kickSubType == 1 && !enableFreeKickPower) {
        power = (dist > 6.0) ? 1.5 : 4.0;
    } else if (data->kickSubType == 4) {
        // Clearance kick - use defensive_clear_power
        double clearPower;
        get_parameter("strategy.defensive_clear_power", clearPower);
        power = clearPower;
    } else if (data->kickSubType == 3 && deflectionWeight > 0.0) {
        double basePower = (dist > 6.0) ? 1.5 : 4.0;
        power = basePower + deflectionBoost;
    } else {
        if (dist > 6.0) {
            power = 1.5;
        } else {
    ...
    kickMsg.power = power;
    kickMsg.leg = data->kickLeg;
    kickMsg.kick_subtype = data->kickSubType;
...
void Brain::handleSpecialStates() {
    ...
    if (gameState == "PLAY" && gameSubStateType == "FREE_KICK" && isFreekickKickoffSide) {
        ...
        // Set kickSubType=1 for free-kick power shot (was previously done by broken Script node in XML)
        data->kickSubType = 1;
...
vector<int> Brain::getOnCourtPlayerIds(bool includeSelf) {
    vector<int> ids = {};
    int numPlayers = min(config->numOfPlayers, HL_MAX_NUM_PLAYERS);
    for (int i = 0; i < numPlayers; i++) {
        int pid = i + 1;
        if (!includeSelf && pid == config->playerId) continue;
        if (data->penalty[i] == PENALTY_NONE) ids.push_back(pid);
    }
    return ids;
}

vector<int> Brain::getReachableTeammateIds() {
    vector<int> ids = {};
    int numPlayers = min(config->numOfPlayers, HL_MAX_NUM_PLAYERS);
    for (int i = 0; i < numPlayers; i++) {
        int pid = i + 1;
        if (pid == config->playerId) continue;
        if (data->penalty[i] == PENALTY_NONE && data->tmStatus[i].isAlive) {
            ids.push_back(pid);
        }
    }
    return ids;
...
double Brain::getTeamSignalTendency(int cid) {
    int baseCid = TeamSignal::baseCid(cid);
    ostringstream key;
    key << "strategy.team_signals.tendency.cid_" << setw(4) << setfill('0') << baseCid;
    double tendency = 0.0;
    get_parameter(key.str(), tendency);
    if (tendency < 0.0) return 0.0;
    if (tendency > 1.0) return 1.0;
    return tendency;
}

bool Brain::shouldSendTeamSignal(int cid, bool force) {
    if (!data || cid == TeamSignal::NO_COMMAND) return false;
    if (!TeamSignal::isImplemented(cid)) return false;

    bool enabled = true;
    get_parameter("strategy.team_signals.enabled", enabled);
    if (!enabled && !force) return false;

    int cooldownMs = 1500;
    get_parameter("strategy.team_signals.cooldown_ms", cooldownMs);
    if (!force && cid == data->tmLastSignalSent && msecsSince(data->tmLastSignalSendTime) < cooldownMs) {
        return false;
    }

    double tendency = getTeamSignalTendency(cid);
    if (!force) {
        if (tendency <= 0.0) return false;
        if (tendency < 1.0) {
            double roll = static_cast<double>(rand()) / static_cast<double>(RAND_MAX);
            if (roll > tendency) return false;
        }
    }
...
bool Brain::queueTeamCommand(int cmd) {
    if (!data || cmd == 0) return false;

    auto now = get_clock()->now();
    int nextCmdId = 0;
    {
        std::lock_guard<std::mutex> lock(data->tmPendingCommandsMutex);
        for (const auto &pending : data->tmPendingCommands) {
            if (pending.first == cmd) {
                return false;
            }
        }
        if (data->tmPendingCommands.size() >= 8) {
            data->tmPendingCommands.pop_front();
        }

        nextCmdId = max(data->tmCmdId, data->tmMyCmdId) + 1;
        if (!data->tmPendingCommands.empty()) {
            nextCmdId = max(nextCmdId, data->tmPendingCommands.back().second + 1);
        }
        data->tmPendingCommands.push_back({cmd, nextCmdId});
    }

    data->tmCmdId = nextCmdId;
    data->tmMyCmdId = nextCmdId;
    data->tmLastCmdChangeTime = now;
    return true;
}

bool Brain::sendTeamSignal(int cid, bool force) {
    if (!shouldSendTeamSignal(cid, force)) return false;

    auto now = get_clock()->now();
    if (!queueTeamCommand(cid)) return false;

    data->tmLastSignalSent = cid;
    data->tmLastSignalSendTime = now;

    log->setTimeNow();
    log->log("debug/teamSignal/send", rerun::TextLog(format("sent CID %04d cmdId=%d", cid, data->tmMyCmdId)));
    return true;
}

void Brain::handleReceivedTeamSignal(int cid, int senderId) {
    auto log_ = [=](string msg) {
        log->setTimeNow();
        log->log("debug/teamSignal/receive", rerun::TextLog(msg));
    };

    if (!TeamSignal::isImplemented(cid)) {
        log_(format("CID %04d from P%d is not implemented in v2.2", cid, senderId));
        return;
    }

    int baseCid = TeamSignal::baseCid(cid);
    int par = TeamSignal::param(cid);
    int senderIdx = senderId - 1;
    bool senderValid = senderIdx >= 0 && senderIdx < HL_MAX_NUM_PLAYERS;
    bool targetValid = par > 0 && par <= HL_MAX_NUM_PLAYERS;
    auto now = get_clock()->now();

    log_(format("received CID %04d base=%04d par=%d from P%d", cid, baseCid, par, senderId));

    auto resetTwoToOne = [&]() {
        data->twoToOneRole = TwoToOneRole::INACTIVE;
        data->twoToOneStage = TwoToOneStage::IDLE;
        data->twoToOneCarrierId = -1;
        data->twoToOneSupportId = -1;
        data->twoToOneFlankSign = 0;
        data->twoToOneOpponentCount = 0;
        data->twoToOneSupportReady = false;
        data->twoToOneRunnerReady = false;
        data->twoToOneStartSignalSent = false;
        data->twoToOneReadySignalSent = false;
        data->twoToOneRunReadySignalSent = false;
        data->twoToOneFirstPassSignalSent = false;
        data->twoToOneReturnPassSignalSent = false;
        data->twoToOneStartTime = now;
        data->twoToOneStageTime = now;
        data->twoToOneLastSignalTime = now;
    };
    ...
    switch (baseCid) {
        case TeamSignal::TAKE_LEAD:
        case TeamSignal::HOLD_LEAD:
            data->tmImLead = false;
            tree->setEntry<bool>("is_lead", false);
            break;

        case TeamSignal::RELEASE_LEAD:
            // The normal cost-rank logic in handleCooperation recalculates lead every tick.
            break;

        case TeamSignal::REQUEST_BALL:
        case TeamSignal::PASS_TO_ME:
            data->tmPassRequested = true;
            data->tmPassTargetId = senderId;
            data->tmPassSignalTime = now;
            break;

        case TeamSignal::PASS_TO_PLAYER:
            if (!targetValid) {
                log_(format("Ignoring PASS_TO_PLAYER with invalid target P%d", par));
                break;
            }
            data->tmPassRequested = true;
            ...
            if (par == config->playerId) {
                ...
                data->tmPassFromId = senderId;
            }
            break;

        case TeamSignal::DOING_PASS:
            if (!targetValid) {
                log_(format("Ignoring DOING_PASS with invalid target P%d", par));
                break;
            }
            data->tmPassTargetId = par;
            data->tmPassSignalTime = now;
        ...
        case TeamSignal::DOING_CROSS:
            data->tmCrossIncoming = true;
            data->tmCrossSignalTime = now;
            break;

        case TeamSignal::WALL_PASS_START:
            if (!targetValid || par != config->playerId || !senderValid) {
                break;
            }
            data->twoToOneRole = TwoToOneRole::SUPPORT;
            data->twoToOneStage = TwoToOneStage::SUPPORT_MOVING;
            data->twoToOneCarrierId = senderId;
            data->twoToOneSupportId = config->playerId;
            data->twoToOneSupportReady = false;
            data->twoToOneRunnerReady = false;
            data->twoToOneStartSignalSent = true;
            data->twoToOneReadySignalSent = false;
            data->twoToOneRunReadySignalSent = false;
            data->twoToOneFirstPassSignalSent = false;
            data->twoToOneReturnPassSignalSent = false;
            ...
            data->tmPassFromId = senderId;
            tree->setEntry<bool>("is_lead", false);
            log_(format("wall-pass start accepted: carrier P%d -> support P%d", senderId, par));
            break;

        case TeamSignal::WALL_PASS_READY:
            if (
                targetValid && par == config->playerId &&
                data->twoToOneRole == TwoToOneRole::CARRIER &&
                data->twoToOneSupportId == senderId
            ) {
                data->twoToOneSupportReady = true;
                data->twoToOneLastSignalTime = now;
                log_(format("wall-pass support P%d ready", senderId));
            }
            break;

        case TeamSignal::WALL_PASS_RUN_READY:
            if (
                targetValid && par == config->playerId &&
                data->twoToOneRole == TwoToOneRole::SUPPORT &&
                data->twoToOneCarrierId == senderId
            ) {
                data->twoToOneRunnerReady = true;
                data->twoToOneStage = TwoToOneStage::RETURN_PASS;
                data->twoToOneStageTime = now;
                data->twoToOneLastSignalTime = now;
                log_(format("wall-pass carrier P%d run-ready", senderId));
            }
            break;

        case TeamSignal::WALL_PASS_COMPLETE:
            if (
                data->twoToOneRole != TwoToOneRole::INACTIVE &&
                (senderId == data->twoToOneCarrierId || senderId == data->twoToOneSupportId || par == config->playerId)
            ) {
                resetTwoToOne();
                log_(format("wall-pass complete/reset from P%d", senderId));
            }
            break;

        case TeamSignal::DANGER:
        ...
        case TeamSignal::PRESS:
            data->tmDefensiveMode = "press";
            data->tmDefensiveModeTime = now;
            break;

        case TeamSignal::FALL_BACK:
            data->tmDefensiveMode = "retreat";
            data->tmDefensiveModeTime = now;
            break;

        case TeamSignal::CLEAR:
            data->tmClearRequested = true;
            data->tmClearRequestTime = now;
            break;

        case TeamSignal::SPREAD:
            data->tmFormationAdjust = "spread";
            data->tmFormationAdjustTime = now;
            break;

        case TeamSignal::COMPACT:
            data->tmFormationAdjust = "compact";
            data->tmFormationAdjustTime = now;
            break;

        case TeamSignal::HOLD_POSITION:
            data->tmDefensiveMode = "hold";
            data->tmDefensiveModeTime = now;
            break;

        case TeamSignal::BALL_LOST:
            if (senderValid) data->tmBallLostBy[senderIdx] = true;
            break;

        case TeamSignal::BALL_FOUND:
            if (senderValid) data->tmBallLostBy[senderIdx] = false;
            break;

        case TeamSignal::STUCK:
            if (senderValid) data->tmStuckPlayers[senderIdx] = true;
            break;

        case TeamSignal::UNSTUCK:
            if (senderValid) data->tmStuckPlayers[senderIdx] = false;
            break;

        case TeamSignal::FALLEN:
            if (senderValid) data->tmFallenPlayers[senderIdx] = true;
            break;

        case TeamSignal::RECOVERED:
            if (senderValid) data->tmFallenPlayers[senderIdx] = false;
            break;
        ...
        default:
            log_(format("CID %04d from P%d has no behavior hook", cid, senderId));
            break;
    }
}
...
void Brain::handleCooperation() {
    ...
    // P2-2 fix: lead 状态黏性. 已是 lead 时需要队友 cost 明显更低才交出 lead, 防止近似 cost 下每 tick 抖动.
    double leadStickyMargin = data->tmImLead ? 0.5 : 0.0;
    if (
        (tmMinCost < BALL_CONTROL_COST_THRESHOLD && data->tmMyCost > tmMinCost + leadStickyMargin)
        || myCostRank >= 2
    ) {
    ...
    auto processReceivedCommand = [&](int cmd, int senderId) {
        if (cmd == 0) return;
        log_(format("received cmd %d from teammate %d", cmd, senderId));
        if (TeamSignal::isExtended(cmd)) {
            handleReceivedTeamSignal(cmd, senderId);
        } else if (cmd == 100) { // 队友要控球, legacy command
            data->tmImLead = false;
            tree->setEntry<bool>("is_lead", false);
            log_("teammate wants to take lead, i'll assist");
        } else if (cmd > 10 && cmd < 20) { // legacy goalie switch
    ...
    vector<tuple<int, int, int>> pendingCommands = {};
    {
        std::lock_guard<std::mutex> lock(data->tmReceivedCmdMutex);
        while (!data->tmReceivedCmdQueue.empty()) {
            pendingCommands.push_back(data->tmReceivedCmdQueue.front());
            data->tmReceivedCmdQueue.pop_front();
        }
    }
    if (pendingCommands.empty() && data->tmReceivedCmd != 0) {
        pendingCommands.push_back({data->tmReceivedCmd, data->tmReceivedCmdFromId, data->tmCmdId});
    }
    for (const auto &pendingCommand : pendingCommands) {
        int cmd, senderId, senderCmdId;
        tie(cmd, senderId, senderCmdId) = pendingCommand;
        processReceivedCommand(cmd, senderId);
    }
    if (!pendingCommands.empty()) {
        data->tmReceivedCmd = 0;
        data->tmReceivedCmdFromId = -1;
    }

    static bool leadSignalInitialized = false;
    static bool lastLeadForSignal = false;
    if (data->tmImAlive) {
        if (data->tmImLead) {
            sendTeamSignal(leadSignalInitialized && lastLeadForSignal ? TeamSignal::HOLD_LEAD : TeamSignal::TAKE_LEAD);
        } else if (leadSignalInitialized && lastLeadForSignal) {
            sendTeamSignal(TeamSignal::RELEASE_LEAD);
        }
    }
    leadSignalInitialized = true;
    lastLeadForSignal = data->tmImLead;

    static bool ballSignalInitialized = false;
    static bool lastBallDetectedForSignal = false;
    if (data->tmImAlive && ballSignalInitialized && data->ballDetected != lastBallDetectedForSignal) {
        sendTeamSignal(data->ballDetected ? TeamSignal::BALL_FOUND : TeamSignal::BALL_LOST);
    }
    ballSignalInitialized = true;
    lastBallDetectedForSignal = data->ballDetected;

    static rclcpp::Time lastGoalieSwitchCmdTime = get_clock()->now();
    if (
        data->tmImAlive 
        && tree->getEntry<string>("player_role") == "goal_keeper"
        && data->tmImLead 
        && msecsSince(lastGoalieSwitchCmdTime) > CMD_COOLDOWN 
    ) {
        ...
        if (minIndex >= 0 && myDist > maxDist) {
            int legacySwitchCmd = 10 + minIndex + 1;
            queueTeamCommand(legacySwitchCmd);
            lastGoalieSwitchCmdTime = get_clock()->now();
            tree->setEntry<string>("player_role", "striker");
...
double Brain::calcClearDir() {
    // Calculate clearance direction: kick to sideline away from own goal
    double ballX = data->ball.posToField.x;
    double ballY = data->ball.posToField.y;
    // If ball is on left side of field, kick right; if on right, kick left
    double targetY = (ballY > 0) ? -4.5 : 4.5;  // Aim for sideline
    double targetX = std::max(ballX + 2.0, 0.0);  // Kick upfield
    return atan2(targetY - ballY, targetX - ballX);
}
...
bool Brain::isAngleGood(double goalPostMargin, string type) {
    ...
    // P0-1 fix: 原来的比较方向反了 (窗口小于 120° 时反而用更大的 margin 0.5, 而正常比赛中窗口几乎总是
    // 小于 120°), 导致远距离射门窗口只有 ±6-8°, 机器人绕球转很久都满足不了. 改为: 离球门很近、窗口很大时
    // 才收紧 margin 防止打中门柱; 远距离用调用方传入的 margin.
...
void Brain::gameControlCallback(const game_controller_interface::msg::GameControlData &msg)
{
    ...
    auto stopForBadGc = [&](const string &reason) {
        prtErr(reason);
        tree->setEntry<string>("gc_game_state", "SET");
        tree->setEntry<string>("gc_game_sub_state_type", "NONE");
        tree->setEntry<string>("gc_game_sub_state", "SET");
        tree->setEntry<bool>("gc_is_kickoff_side", false);
        tree->setEntry<bool>("gc_is_sub_state_kickoff_side", false);
        tree->setEntry<bool>("gc_is_under_penalty", true);
        client->setVelocity(0.0, 0.0, 0.0, false, false, false);
    };
    ...
    int gameStateIndex = static_cast<int>(msg.state);
    string gameState = "SET";
    if (gameStateIndex >= 0 && gameStateIndex < static_cast<int>(gameStateMap.size())) {
        gameState = gameStateMap[gameStateIndex];
    } else {
        prtErr(format("invalid GameController state %d, fail-stop as SET", gameStateIndex));
    }
    tree->setEntry<string>("gc_game_state", gameState);
    if (gameState == "SET" && lastGameState != "SET") {
        client->setVelocity(0.0, 0.0, 0.0, false, false, false);
    }
    ...
    int gameSubStateIndex = static_cast<int>(msg.secondary_state_info[1]);
    if (gameSubStateIndex >= static_cast<int>('0') && gameSubStateIndex <= static_cast<int>('2')) {
        gameSubStateIndex -= static_cast<int>('0');
    }
    string gameSubState = "STOP";
    if (gameSubStateIndex >= 0 && gameSubStateIndex < static_cast<int>(gameSubStateMap.size())) {
        gameSubState = gameSubStateMap[gameSubStateIndex];
    } else {
        prtErr(format("invalid GameController sub-state info %d, fail-stop as STOP", gameSubStateIndex));
    }
    ...
    else
    {
        ...
        stopForBadGc(format("received invalid game controller message team0 %d, team1 %d, teamId %d",
            msg.teams[0].team_number, msg.teams[1].team_number, config->teamId));
        return;
    }

    if (config->playerId < 1 || config->playerId > HL_MAX_NUM_PLAYERS) {
        stopForBadGc(format("invalid config playerId %d", config->playerId));
        return;
    }
    ...
    for (int i = 0; i < HL_MAX_NUM_PLAYERS; i++) {
        ...
        if (i < static_cast<int>(myTeamInfo.players.size()) && static_cast<int>(myTeamInfo.players[i].red_card_count) > 0) {
            data->penalty[i] = PENALTY_SUBSTITUTE;
        }
        ...
        if (i < static_cast<int>(oppoTeamInfo.players.size()) && static_cast<int>(oppoTeamInfo.players[i].red_card_count) > 0) {
            data->oppoPenalty[i] = PENALTY_SUBSTITUTE;
        }
...
void Brain::recoveryStateCallback(const booster_interface::msg::RawBytesMsg &msg)
{
    ...
    try
    {
        ...
        if (buffer.size() < sizeof(RobotRecoveryStateData)) {
            RCLCPP_WARN(get_logger(), "Invalid recovery state payload size: %zu", buffer.size());
            return;
        }
        ...
        if (static_cast<int>(recoveryState.state) < 0 || static_cast<size_t>(recoveryState.state) >= recoveryStateMap.size()) {
            RCLCPP_WARN(get_logger(), "Invalid recovery state value: %d", static_cast<int>(recoveryState.state));
            return;
        }
        RobotRecoveryState lastRecoveryState = this->data->recoveryState;
        ...
        if (lastRecoveryState != RobotRecoveryState::HAS_FALLEN && this->data->recoveryState == RobotRecoveryState::HAS_FALLEN) {
            sendTeamSignal(TeamSignal::FALLEN);
        } else if (lastRecoveryState == RobotRecoveryState::HAS_FALLEN && this->data->recoveryState == RobotRecoveryState::IS_READY) {
            sendTeamSignal(TeamSignal::RECOVERED);
        }
...
void Brain::detectProcessGoalposts(const vector<GameObject> &goalpostObjs)
{
    ...
    // GlanceAtGoal (P1-1): 看到对方半场的门柱, 认为瞄准信息已刷新
    for (const auto &post : goalposts) {
        if (post.posToField.x > 0) {
            data->lastGoalObservationTime = get_clock()->now();
            break;
        }
    }
