...
bool isTwoToOneOpponent(const GameObject &obj, Brain *brain) {
    return obj.label == "Opponent" || (brain->config->treatPersonAsRobot && obj.label == "Person");
}

int chooseTwoToOneFlankSign(const Point2D &opponentPos, Brain *brain) {
    const auto fd = brain->config->fieldDimensions;
    double leftSpace = fd.width / 2.0 - opponentPos.y;
    double rightSpace = opponentPos.y + fd.width / 2.0;
    return leftSpace >= rightSpace ? 1 : -1;
}

Pose2D clampTwoToOneTarget(Pose2D target, Brain *brain) {
    const auto fd = brain->config->fieldDimensions;
    const double xMin = -fd.length / 2.0 + 0.6;
    const double xMax = fd.length / 2.0 - 0.7;
    const double yMin = -fd.width / 2.0 + 0.7;
    const double yMax = fd.width / 2.0 - 0.7;
    target.x = std::min(std::max(target.x, xMin), xMax);
    target.y = std::min(std::max(target.y, yMin), yMax);
    return target;
}

Pose2D calcTwoToOneSupportTarget(Brain *brain, const Point2D &opponentPos, int flankSign) {
    double xOffset = 0.8;
    double yOffset = 1.2;
    brain->get_parameter("strategy.two_to_one.support_flank_x_offset", xOffset);
    brain->get_parameter("strategy.two_to_one.support_flank_y_offset", yOffset);

    Pose2D target;
    target.x = opponentPos.x + xOffset;
    target.y = opponentPos.y + flankSign * yOffset;
    target.theta = atan2(brain->data->ball.posToField.y - target.y, brain->data->ball.posToField.x - target.x);
    return clampTwoToOneTarget(target, brain);
}

Pose2D calcTwoToOneRunTarget(Brain *brain, const Point2D &opponentPos, int flankSign) {
    double xOffset = 1.4;
    double yOffset = 0.45;
    brain->get_parameter("strategy.two_to_one.runner_x_offset", xOffset);
    brain->get_parameter("strategy.two_to_one.runner_y_offset", yOffset);
...
bool findTwoToOneCorridorOpponent(Brain *brain, GameObject &bestOpponent, int &count) {
    count = 0;
    double rangeThreshold = 5.0;
    double corridorWidth = 2.0;
    brain->get_parameter("strategy.two_to_one.execution_distance_threshold", rangeThreshold);
    brain->get_parameter("strategy.two_to_one.forward_corridor_width", corridorWidth);

    const auto ballPos = brain->data->ball.posToField;
    double bestRange = std::numeric_limits<double>::max();
    for (const auto &robot : brain->data->getRobots()) {
        if (!isTwoToOneOpponent(robot, brain)) continue;
        if (robot.range > rangeThreshold) continue;

        const double forward = robot.posToField.x - ballPos.x;
        const double lateral = fabs(robot.posToField.y - ballPos.y);
        if (forward < -0.4 || forward > rangeThreshold) continue;
        if (lateral > corridorWidth / 2.0) continue;

        count++;
        if (robot.range < bestRange) {
            bestRange = robot.range;
            bestOpponent = robot;
        }
    }
    return count == 1;
...
void resetTwoToOneTactic(Brain *brain) {
    auto now = brain->get_clock()->now();
    brain->data->twoToOneRole = TwoToOneRole::INACTIVE;
    brain->data->twoToOneStage = TwoToOneStage::IDLE;
    brain->data->twoToOneCarrierId = -1;
    brain->data->twoToOneSupportId = -1;
    brain->data->twoToOneFlankSign = 0;
    brain->data->twoToOneOpponentCount = 0;
    brain->data->twoToOneSupportReady = false;
    brain->data->twoToOneRunnerReady = false;
    brain->data->twoToOneStartSignalSent = false;
    brain->data->twoToOneReadySignalSent = false;
    brain->data->twoToOneRunReadySignalSent = false;
    brain->data->twoToOneFirstPassSignalSent = false;
    brain->data->twoToOneReturnPassSignalSent = false;
    brain->data->twoToOneStartTime = now;
    brain->data->twoToOneStageTime = now;
    brain->data->twoToOneLastSignalTime = now;
}

bool isTwoToOneTacticTimedOut(Brain *brain) {
    int maxTotalMs = 7000;
    brain->get_parameter("strategy.two_to_one.max_total_time_ms", maxTotalMs);
    return brain->data->twoToOneRole != TwoToOneRole::INACTIVE &&
           brain->msecsSince(brain->data->twoToOneStartTime) > maxTotalMs;
}

bool isTwoToOnePartnerStale(Brain *brain) {
    int partnerId = -1;
    if (brain->data->twoToOneRole == TwoToOneRole::CARRIER) {
        partnerId = brain->data->twoToOneSupportId;
    } else if (brain->data->twoToOneRole == TwoToOneRole::SUPPORT) {
        partnerId = brain->data->twoToOneCarrierId;
    }

    if (partnerId <= 0 || partnerId > HL_MAX_NUM_PLAYERS) return true;
    int commTimeoutMs = 1400;
    brain->get_parameter("strategy.two_to_one.comm_timeout_ms", commTimeoutMs);
    const auto &status = brain->data->tmStatus[partnerId - 1];
    return !status.isAlive || brain->msecsSince(status.timeLastCom) > commTimeoutMs;
}

bool isTwoToOneAbortNeeded(Brain *brain) {
    if (brain->data->twoToOneRole == TwoToOneRole::INACTIVE) return false;
    if (brain->data->recoveryState != RobotRecoveryState::IS_READY) return true;
    if (brain->tree->getEntry<string>("gc_game_state") != "PLAY") return true;
    if (brain->tree->getEntry<bool>("ball_out")) return true;
    bool ballKnown = brain->tree->getEntry<bool>("ball_location_known") ||
                     brain->tree->getEntry<bool>("tm_ball_pos_reliable");
    if (!ballKnown) return true;
    if (
        brain->data->ballDetected &&
        brain->data->ball.confidence < brain->get_parameter("strategy.two_to_one.min_ball_confidence").as_double()
    ) {
        return true;
    }
    if (isTwoToOneTacticTimedOut(brain) || isTwoToOnePartnerStale(brain)) return true;

    bool abortExtraOpponents = true;
    brain->get_parameter("strategy.two_to_one.abort_extra_opponents", abortExtraOpponents);
    if (abortExtraOpponents) {
        GameObject unused;
        int corridorCount = 0;
        findTwoToOneCorridorOpponent(brain, unused, corridorCount);
        if (corridorCount > 1) return true;
    }

    int teamSignalTtlMs = 2500;
    brain->get_parameter("strategy.team_signals.ttl_ms", teamSignalTtlMs);
    if (brain->data->tmDangerLevel >= 7 && brain->msecsSince(brain->data->tmDangerTime) < teamSignalTtlMs) {
        return true;
    }
...
void abortTwoToOneTactic(Brain *brain, const string &reason) {
    ...
    resetTwoToOneTactic(brain);
}

// P0-3 fix: 踢球(crabWalk 穿球)时不再朝 ball.yawToRobot 直走, 而是走向球沿 kickDir 方向后方 0.3m 的点,
// 使穿球路径与规划的踢球方向一致. 为防止 kickDir 过期/未设置(如手动模式)导致踢空, 限制在球方向 ±0.35rad 内.
double calcKickApproachAngle(Brain *brain) {
    double kickDirF = brain->data->isClearance ? brain->calcClearDir() : brain->data->kickDir;
    Pose2D beyond;
    beyond.x = brain->data->ball.posToField.x + 0.3 * cos(kickDirF);
    beyond.y = brain->data->ball.posToField.y + 0.3 * sin(kickDirF);
    Pose2D beyond_r = brain->data->field2robot(beyond);
    double angle = atan2(beyond_r.y, beyond_r.x);
    double ballYaw = brain->data->ball.yawToRobot;
    return cap(angle, ballYaw + 0.35, ballYaw - 0.35);
}
...
void BrainTree::init()
{
    ...
    REGISTER_BUILDER(TwoToOneSupport)
    REGISTER_BUILDER(TwoToOneRun)
...
NodeStatus TwoToOneSupport::tick() {
    if (brain->data->twoToOneRole != TwoToOneRole::SUPPORT) {
        brain->client->setVelocity(0, 0, 0);
        return NodeStatus::SUCCESS;
    }
    if (isTwoToOneAbortNeeded(brain)) {
        abortTwoToOneTactic(brain, "support abort");
        return NodeStatus::SUCCESS;
    }

    GameObject opponent;
    int corridorCount = 0;
    Point2D opponentPos = brain->data->twoToOneOpponentPos;
    if (findTwoToOneCorridorOpponent(brain, opponent, corridorCount)) {
        opponentPos = Point2D{opponent.posToField.x, opponent.posToField.y};
        brain->data->twoToOneOpponentPos = opponentPos;
        brain->data->twoToOneOpponentCount = corridorCount;
    } else if (brain->data->twoToOneFlankSign == 0) {
        opponentPos = Point2D{brain->data->ball.posToField.x + 1.0, brain->data->ball.posToField.y};
        brain->data->twoToOneOpponentPos = opponentPos;
    }

    if (brain->data->twoToOneFlankSign == 0) {
        brain->data->twoToOneFlankSign = chooseTwoToOneFlankSign(opponentPos, brain);
    }

    Pose2D targetPose = calcTwoToOneSupportTarget(brain, opponentPos, brain->data->twoToOneFlankSign);
    brain->data->twoToOneSupportTarget = targetPose;

    double distTolerance = getInput<double>("dist_tolerance").value();
    brain->get_parameter("strategy.two_to_one.ready_radius", distTolerance);
    double thetaTolerance = getInput<double>("theta_tolerance").value();
    double dist = norm(targetPose.x - brain->data->robotPoseToField.x, targetPose.y - brain->data->robotPoseToField.y);
    double deltaTheta = fabs(toPInPI(targetPose.theta - brain->data->robotPoseToField.theta));

    if (dist < distTolerance && deltaTheta < thetaTolerance) {
        brain->client->setVelocity(0, 0, 0);
        if (!brain->data->twoToOneReadySignalSent && brain->data->twoToOneCarrierId > 0) {
            brain->sendTeamSignal(TeamSignal::withParam(TeamSignal::WALL_PASS_READY, brain->data->twoToOneCarrierId), true);
            brain->data->twoToOneReadySignalSent = true;
            brain->data->twoToOneStage = TwoToOneStage::FIRST_PASS;
            brain->data->twoToOneStageTime = brain->get_clock()->now();
        }
        return NodeStatus::SUCCESS;
    }
    ...
    brain->client->moveToPoseOnField3(targetPose.x, targetPose.y, targetPose.theta, 1.0, 0.45, vxLimit, vyLimit, vthetaLimit, 0.2, 0.2, 0.18, true);
    return NodeStatus::SUCCESS;
}

NodeStatus TwoToOneRun::tick() {
    if (brain->data->twoToOneRole != TwoToOneRole::CARRIER) {
        brain->client->setVelocity(0, 0, 0);
        return NodeStatus::SUCCESS;
    }
    if (isTwoToOneAbortNeeded(brain)) {
        abortTwoToOneTactic(brain, "carrier run abort");
        return NodeStatus::SUCCESS;
    }

    Pose2D targetPose = brain->data->twoToOneRunTarget;
    if (brain->data->twoToOneFlankSign == 0) {
        brain->data->twoToOneFlankSign = chooseTwoToOneFlankSign(brain->data->twoToOneOpponentPos, brain);
    }
    targetPose = calcTwoToOneRunTarget(brain, brain->data->twoToOneOpponentPos, brain->data->twoToOneFlankSign);
    targetPose.theta = 0.0;
    brain->data->twoToOneRunTarget = targetPose;
    ...
    if (dist < distTolerance && deltaTheta < thetaTolerance) {
        brain->client->setVelocity(0, 0, 0);
        if (!brain->data->twoToOneRunReadySignalSent && brain->data->twoToOneSupportId > 0) {
            brain->sendTeamSignal(TeamSignal::withParam(TeamSignal::WALL_PASS_RUN_READY, brain->data->twoToOneSupportId), true);
            brain->data->twoToOneRunReadySignalSent = true;
            brain->data->twoToOneRunnerReady = true;
            brain->data->twoToOneStageTime = brain->get_clock()->now();
        }
        return NodeStatus::SUCCESS;
    }
    ...
    double vyLimit = getInput<double>("vy_limit").value();
    double vthetaLimit = getInput<double>("vtheta_limit").value();
    brain->client->moveToPoseOnField3(targetPose.x, targetPose.y, targetPose.theta, 0.9, 0.45, vxLimit, vyLimit, vthetaLimit, 0.2, 0.2, 0.2, true);
    return NodeStatus::SUCCESS;
}
...
NodeStatus StrikerDecide::tick() {
    ...
    // Band split: RL VisualKick 只负责 [dist_min, dist_max] 区间的接近+踢球, 脚下区域永远留给
    // 确定性的 Kick 节点. 这里强制下限不小于 kick_range*1.2, 即使配置写错也不会遮蔽正常踢球.
    autoVisualKickEnableDistMin = max(autoVisualKickEnableDistMin, KICK_RANGE * 1.2);
    int visualKickReentryCooldownMs = 4000;
    brain->get_parameter("strategy.auto_visual_kick_reentry_cooldown_ms", visualKickReentryCooldownMs);
    double kickDir = brain->data->kickDir;
    ...
    const double SHOOT_Y_RANGE = 0.12;
    const double SHOOT_X_MAX = 0.6;
    const double SHOOT_X_MIN = 0.2;
    const double STRONG_SHOOT_X_MIN = 1.0;
    // --- Leg selection ---
    // Ball in robot frame: +Y = left, -Y = right. Centered threshold = 0.01m
    brain->data->kickLeg = 0;
    if (ballY > 0.01) {
        brain->data->kickLeg = 1;
    } else if (ballY < -0.01) {
        brain->data->kickLeg = 0;
    }
    ...
    // P0-1/P0-3 fix: 静态阈值 0.1 -> 0.2 (更快满足), 过线判定 M_PI/6 -> 0.25 (限制出脚时的方向误差)
    bool reachedKickDir =
    ...
    int teamSignalTtlMs = 2500;
    brain->get_parameter("strategy.team_signals.ttl_ms", teamSignalTtlMs);
    bool retreatSignalActive =
        brain->data->tmDefensiveMode == "retreat" &&
        brain->msecsSince(brain->data->tmDefensiveModeTime) < teamSignalTtlMs;
    bool pressSignalActive =
        brain->data->tmDefensiveMode == "press" &&
        brain->msecsSince(brain->data->tmDefensiveModeTime) < teamSignalTtlMs;
    bool clearSignalActive =
        brain->data->tmClearRequested &&
        brain->msecsSince(brain->data->tmClearRequestTime) < teamSignalTtlMs;

    auto setKickDirToPoint = [&](const Point2D &target) {
        brain->data->kickType = "shoot";
        brain->data->isClearance = false;
        brain->data->kickSubType = 0;
        brain->data->kickDir = atan2(target.y - ball.posToField.y, target.x - ball.posToField.x);
    };

    auto ballReadyForTacticKick = [&]() {
        const double passDelta = fabs(toPInPI(brain->data->kickDir - dir_rb_f));
        return brain->data->ballDetected &&
               ball.range < KICK_RANGE &&
               fabs(ballYaw) < KICK_THETA_RANGE &&
               passDelta < 0.18;
    };

    auto selectTwoToOneSupportId = [&](const Pose2D &supportTarget) {
        int bestPid = -1;
        double bestScore = std::numeric_limits<double>::max();
        for (int pid : brain->getReachableTeammateIds()) {
            int idx = pid - 1;
            if (idx < 0 || idx >= HL_MAX_NUM_PLAYERS) continue;
            const auto &status = brain->data->tmStatus[idx];
            if (!status.isAlive || status.role == "goal_keeper") continue;
            if (brain->data->tmFallenPlayers[idx] || brain->data->tmStuckPlayers[idx]) continue;
            double score = norm(status.robotPoseToField.x - supportTarget.x, status.robotPoseToField.y - supportTarget.y);
            if (score < bestScore) {
                bestScore = score;
                bestPid = pid;
            }
    ...
    bool twoToOneEnabled = false;
    brain->get_parameter("strategy.two_to_one.enabled", twoToOneEnabled);
    if (twoToOneEnabled && brain->data->twoToOneRole != TwoToOneRole::INACTIVE) {
        if (isTwoToOneAbortNeeded(brain)) {
            abortTwoToOneTactic(brain, "striker decision abort");
            newDecision = "chase";
            color = 0xFF0000FF;
        } else if (brain->data->twoToOneRole == TwoToOneRole::CARRIER) {
            if (brain->data->twoToOneStage == TwoToOneStage::WAIT_SUPPORT) {
                setKickDirToPoint(Point2D{brain->data->twoToOneSupportTarget.x, brain->data->twoToOneSupportTarget.y});
                int maxSetupMs = 2500;
                brain->get_parameter("strategy.two_to_one.max_setup_time_ms", maxSetupMs);
                if (brain->msecsSince(brain->data->twoToOneStageTime) > maxSetupMs) {
                    abortTwoToOneTactic(brain, "wall-pass setup timeout");
                    newDecision = "chase";
                    color = 0xFF0000FF;
                } else if (brain->data->twoToOneSupportReady) {
                    brain->data->twoToOneStage = TwoToOneStage::FIRST_PASS;
                    brain->data->twoToOneStageTime = now;
                } else {
                    newDecision = ballRange > KICK_RANGE ? "chase" : "adjust";
                    color = 0x00AAFFFF;
                }
            }

            if (newDecision.empty() && brain->data->twoToOneStage == TwoToOneStage::FIRST_PASS) {
                setKickDirToPoint(Point2D{brain->data->twoToOneSupportTarget.x, brain->data->twoToOneSupportTarget.y});
                int firstPassMs = 900;
                brain->get_parameter("strategy.two_to_one.first_pass_kick_ms", firstPassMs);
                if (!brain->data->twoToOneFirstPassSignalSent) {
                    if (ballReadyForTacticKick()) {
                        brain->sendTeamSignal(TeamSignal::withParam(TeamSignal::DOING_PASS, brain->data->twoToOneSupportId), true);
                        brain->data->twoToOneFirstPassSignalSent = true;
                        brain->data->twoToOneStageTime = now;
                        newDecision = "two_to_one_first_pass";
                    } else {
                        newDecision = ballRange > KICK_RANGE ? "chase" : "adjust";
                    }
                } else if (brain->msecsSince(brain->data->twoToOneStageTime) < firstPassMs) {
                    newDecision = "two_to_one_first_pass";
                } else {
                    brain->data->twoToOneStage = TwoToOneStage::RUNNER_MOVING;
                    brain->data->twoToOneStageTime = now;
                    newDecision = "two_to_one_run";
                }
                color = 0x00AAFFFF;
            } else if (newDecision.empty() && brain->data->twoToOneStage == TwoToOneStage::RUNNER_MOVING) {
                double receiveBallRange = 1.0;
                brain->get_parameter("strategy.two_to_one.receive_ball_range", receiveBallRange);
                if (brain->data->twoToOneRunReadySignalSent && brain->data->ballDetected && ballRange < receiveBallRange) {
                    brain->sendTeamSignal(TeamSignal::WALL_PASS_COMPLETE, true);
                    resetTwoToOneTactic(brain);
                } else {
                    newDecision = "two_to_one_run";
            ...
            int carrierId = brain->data->twoToOneCarrierId;
            int carrierIdx = carrierId - 1;
            if (
                carrierIdx >= 0 && carrierIdx < HL_MAX_NUM_PLAYERS &&
                brain->data->twoToOneStage == TwoToOneStage::RETURN_PASS
            ) {
                const auto &carrierStatus = brain->data->tmStatus[carrierIdx];
                Point2D returnTarget{
                    carrierStatus.robotPoseToField.x + 0.25,
                    carrierStatus.robotPoseToField.y
                };
                setKickDirToPoint(returnTarget);
                int returnPassMs = 900;
                brain->get_parameter("strategy.two_to_one.return_pass_kick_ms", returnPassMs);
                if (!brain->data->twoToOneReturnPassSignalSent) {
                    if (ballReadyForTacticKick()) {
                        brain->sendTeamSignal(TeamSignal::withParam(TeamSignal::DOING_PASS, carrierId), true);
                        brain->data->twoToOneReturnPassSignalSent = true;
                        brain->data->twoToOneStageTime = now;
                        newDecision = "two_to_one_return_pass";
                    } else {
                        newDecision = ballRange > KICK_RANGE ? "chase" : "adjust";
                    }
                } else if (brain->msecsSince(brain->data->twoToOneStageTime) < returnPassMs) {
                    newDecision = "two_to_one_return_pass";
                } else {
                    brain->sendTeamSignal(TeamSignal::WALL_PASS_COMPLETE, true);
                    resetTwoToOneTactic(brain);
                ...
                if (
                    carrierIdx >= 0 && carrierIdx < HL_MAX_NUM_PLAYERS &&
                    brain->data->twoToOneSupportReady &&
                    brain->data->ballDetected &&
                    ballRange < receiveBallRange
                ) {
                    const auto &carrierStatus = brain->data->tmStatus[carrierIdx];
                    setKickDirToPoint(Point2D{carrierStatus.robotPoseToField.x + 0.25, carrierStatus.robotPoseToField.y});
                    newDecision = "adjust";
                } else {
                    newDecision = "two_to_one_support";
                }
                color = 0x00AAFFFF;
            }
        }
    ...
    if (
        ...
        brain->data->ballDetected &&
        ball.confidence >= brain->get_parameter("strategy.two_to_one.min_ball_confidence").as_double() &&
        ballRange < 1.4 &&
        !brain->tree->getEntry<bool>("ball_out") &&
        brain->getReachableTeammateIds().size() > 0
    ) {
        GameObject corridorOpponent;
        int corridorCount = 0;
        if (findTwoToOneCorridorOpponent(brain, corridorOpponent, corridorCount)) {
            Point2D opponentPos{corridorOpponent.posToField.x, corridorOpponent.posToField.y};
            int flankSign = chooseTwoToOneFlankSign(opponentPos, brain);
            Pose2D supportTarget = calcTwoToOneSupportTarget(brain, opponentPos, flankSign);
            Pose2D runTarget = calcTwoToOneRunTarget(brain, opponentPos, flankSign);
            int supportId = selectTwoToOneSupportId(supportTarget);
            if (supportId > 0) {
                brain->data->twoToOneRole = TwoToOneRole::CARRIER;
                brain->data->twoToOneStage = TwoToOneStage::WAIT_SUPPORT;
                brain->data->twoToOneCarrierId = brain->config->playerId;
                brain->data->twoToOneSupportId = supportId;
                brain->data->twoToOneFlankSign = flankSign;
                brain->data->twoToOneOpponentCount = corridorCount;
                brain->data->twoToOneOpponentPos = opponentPos;
                brain->data->twoToOneSupportTarget = supportTarget;
                brain->data->twoToOneRunTarget = runTarget;
                brain->data->twoToOneSupportReady = false;
                brain->data->twoToOneRunnerReady = false;
                brain->data->twoToOneStartSignalSent = true;
                brain->data->twoToOneReadySignalSent = false;
                brain->data->twoToOneRunReadySignalSent = false;
                brain->data->twoToOneFirstPassSignalSent = false;
                brain->data->twoToOneReturnPassSignalSent = false;
                brain->data->twoToOneStartTime = now;
                brain->data->twoToOneStageTime = now;
                brain->sendTeamSignal(TeamSignal::withParam(TeamSignal::WALL_PASS_START, supportId), true);
                setKickDirToPoint(Point2D{supportTarget.x, supportTarget.y});
                newDecision = ballRange > KICK_RANGE ? "chase" : "adjust";
                color = 0x00AAFFFF;
                log(format("2v1 wall-pass call: support P%d target=(%.2f, %.2f)", supportId, supportTarget.x, supportTarget.y));
            }
        }
    }

    // P0-2 fix: 一旦进入踢球类决策, 只要球仍在可踢范围内就短暂锁定该决策.
    // 否则机器人自身移动会使 angle 条件抖动, Kick(Stateful) 节点被 _while 反复打断, 表现为"犹豫不踢".
    bool wasKickFamily = (lastDecision == "kick" || lastDecision == "safe_shoot" || lastDecision == "cross");
    if (
        newDecision.empty() && wasKickFamily &&
        brain->data->twoToOneRole == TwoToOneRole::INACTIVE &&
        brain->msecsSince(_kickCommitTime) < 2500 &&
        ballRange <= KICK_RANGE &&
        (brain->data->ballDetected || brain->msecsSince(ball.timePoint) < 500) &&
        !brain->tree->getEntry<bool>("ball_out")
    ) {
        newDecision = lastDecision;
        color = 0x00FF00FF;
    }

    // Glance hold: 正在瞄门观察时保持 glance 决策, 直到观察窗口结束, 防止冷却机制导致 glance 只执行一个 tick.
    int glanceMsecs = 600;
    brain->get_parameter("strategy.glance_at_goal.msecs", glanceMsecs);
    if (newDecision.empty() && lastDecision == "glance" && brain->msecsSince(_glanceStartTime) < glanceMsecs + 300) {
        newDecision = "glance";
        color = 0xFFFFFFFF;
    }

    if (!newDecision.empty()) {
        // Tactic branch already selected the action for this tick.
    } else if (!(iKnowBallPos || tmBallPosReliable))
    {
        newDecision = "find";
        color = 0xFFFFFFFF;
    } else if (retreatSignalActive) {
        newDecision = "defend";
        color = 0xFF0000FF;
    } else if (pressSignalActive && brain->data->tmMyCostRank == 0) {
        newDecision = "chase";
        color = 0xFFCC00FF;
    } else if (
        ...
        RLVisionKick::isMinIntervalSatisfied(visualKickReentryCooldownMs, now) && // 重入冷却, 防止退出后立刻再进
    ...
    else if (ballRange > chaseRangeThreshold * (lastDecision == "chase" ? 0.9 : 1.0))
    {
        ...
        && !avoidKick
        && (brain->data->ballDetected || brain->msecsSince(ball.timePoint) < 400) // P2-1: 接触距离下球常被自身遮挡, 允许 400ms 内的记忆位置
    ...
    )
    {
        if (brain->data->kickType == "cross") newDecision = "cross";
            ...
            bool enableQuickShot;
            double quickshotMaxRange, deflectionWeight;
            bool enableFreekickDeflection, enableStrikerDefense;
            double defensiveRiskTolerance, defensiveStealRange;
            brain->get_parameter("strategy.enable_quickshot", enableQuickShot);
            brain->get_parameter("strategy.quickshot_max_range", quickshotMaxRange);
            brain->get_parameter("strategy.deflection_shot_weight", deflectionWeight);
            brain->get_parameter("strategy.enable_freekick_deflection", enableFreekickDeflection);
            brain->get_parameter("strategy.enable_striker_defense", enableStrikerDefense);
            brain->get_parameter("strategy.defensive_risk_tolerance", defensiveRiskTolerance);
            brain->get_parameter("strategy.defensive_steal_range", defensiveStealRange);
            // --- Reset temporary state ---
            brain->data->isClearance = false;
            brain->data->kickSubType = 0;
            if (clearSignalActive) {
                newDecision = "kick";
                brain->data->isClearance = true;
                brain->data->kickSubType = 4;
            }
            // --- 5-state possession detection ---
            double possMargin, hysterSecs, attackProtDist;
            brain->get_parameter("strategy.possession_margin", possMargin);
            brain->get_parameter("strategy.possession_hysteresis_secs", hysterSecs);
            brain->get_parameter("strategy.attack_protection_dist", attackProtDist);
            int hysterFrames = (int)(hysterSecs * 30.0);
            // P1-2 fix: 用视觉确认的对方机器人计算 "对手距球距离", 而不是原始深度障碍格
            // (深度格会把队友/裁判/门柱当成对手, 在临门一脚时劫持决策进入 steal/defend).
            double oppNearestDist = 10.0;
            for (auto& rbt : brain->data->getRobots()) {
                if (!(rbt.label == "Opponent" || (brain->config->treatPersonAsRobot && rbt.label == "Person"))) continue;
                double d = norm(rbt.posToField.x - ball.posToField.x, rbt.posToField.y - ball.posToField.y);
                if (d < oppNearestDist) oppNearestDist = d;
            }
            auto now = brain->get_clock()->now();
            double dt = brain->msecsSince(brain->data->prevBallTime) / 1000.0;
            if (dt < 0.001) dt = 0.001;
            double ballVx = 0.0, ballVy = 0.0;
            if (brain->data->hasPrevBall) {
                ballVx = (ball.posToField.x - brain->data->prevBallX) / dt;
                ballVy = (ball.posToField.y - brain->data->prevBallY) / dt;
            }
            brain->data->prevBallX = ball.posToField.x;
            brain->data->prevBallY = ball.posToField.y;
            brain->data->prevBallTime = now;
            brain->data->hasPrevBall = true;
            double goalTheirX = brain->config->fieldDimensions.length / 2.0;
            double goalOurX = -brain->config->fieldDimensions.length / 2.0;
            bool ballMovingToTheirGoal = ballVx * (goalTheirX - ball.posToField.x) > 0;
            bool ballMovingToOurGoal = ballVx * (goalOurX - ball.posToField.x) > 0;
            int rawState = 0;
            double ourDist = ballRange;
            if (ourDist < attackProtDist && ballMovingToTheirGoal) {
                rawState = 0; // OUR_POSSESSION
            } else if (oppNearestDist + possMargin < ourDist && ballMovingToOurGoal) {
                rawState = 1; // OPP_POSSESSION
            } else if (ballVx < -0.5 && oppNearestDist < ourDist) {
                rawState = 4; // DANGER
            } else if (ourDist < defensiveStealRange * 2.0 && oppNearestDist < defensiveStealRange * 2.0) {
                rawState = 2; // CONTESTED
            } else {
                rawState = 3; // FREE_BALL
            }
            if (rawState == brain->data->ballState) {
                brain->data->oppPossessionFrames++;
            } else {
                brain->data->oppPossessionFrames = 0;
                brain->data->ballState = rawState;
            }
            bool oppPossessionConfirmed = (brain->data->ballState == 1 && brain->data->oppPossessionFrames >= hysterFrames);
            bool oppHasBall = oppPossessionConfirmed;
            bool dangerActive = (brain->data->ballState == 4 && brain->data->oppPossessionFrames >= hysterFrames);
            // --- Early defense decisions (before attack flow) ---
            if (enableStrikerDefense) {
                if (oppHasBall && ballRange < defensiveStealRange) {
                    newDecision = "steal";
                    brain->data->isClearance = true;
                    brain->data->kickSubType = 4;
                } else if (oppHasBall && ballRange < defensiveStealRange * 2.0) {
                    // P1-2 fix: 骰子结果缓存 1s, 防止 100Hz 每 tick 重掷导致 steal/defend 决策抖动
                    if (brain->msecsSince(_lastRiskRollTime) > 1000) {
                        _lastRiskRoll = (double)rand() / RAND_MAX;
                        _lastRiskRollTime = now;
                    }
                    if (_lastRiskRoll < defensiveRiskTolerance) {
                        newDecision = "steal";
                        brain->data->isClearance = true;
                        brain->data->kickSubType = 4;
                    } else {
                ...
                } else if (oppHasBall || dangerActive) {
                    newDecision = "defend";
                } else if (rawState == 2 && brain->data->tmMyCostRank == 0) {
                    // CONTESTED and we are closest - chase ball
                    newDecision = "chase";
                } else if (rawState == 2) {
                    // CONTESTED and not closest - block opponent
                    newDecision = "defend";
                }
            }
            int signalToSend = TeamSignal::NO_COMMAND;
            if (dangerActive) {
                signalToSend = TeamSignal::BREAKAWAY;
            } else if (newDecision == "steal" && brain->data->isClearance) {
                signalToSend = TeamSignal::CLEAR;
            } else if (newDecision == "defend") {
                signalToSend = TeamSignal::FALL_BACK;
            } else if (oppHasBall) {
                signalToSend = TeamSignal::DANGER + 6;
            } else if (rawState == 2 && brain->data->tmMyCostRank == 0) {
                signalToSend = TeamSignal::PRESS;
            }
            if (signalToSend != TeamSignal::NO_COMMAND) {
                brain->sendTeamSignal(signalToSend);
            }
            // Only proceed to attack flow if no defense decision was made
            if (newDecision.empty()) {
                // GlanceAtGoal (P1-1): 射门前如果瞄准依据已过期 (很久没看到门柱也没成功重定位),
                // 先看一眼球门方向再踢, 防止靠漂移的定位盲射.
                bool glanceEnable = true;
                int glanceMaxAimAge = 4000, glanceCooldown = 5000;
                brain->get_parameter("strategy.glance_at_goal.enable", glanceEnable);
                brain->get_parameter("strategy.glance_at_goal.max_aim_age_ms", glanceMaxAimAge);
                brain->get_parameter("strategy.glance_at_goal.cooldown_ms", glanceCooldown);
                bool aimFresh =
                    brain->msecsSince(brain->data->lastGoalObservationTime) < glanceMaxAimAge ||
                    brain->msecsSince(brain->data->lastSuccessfulLocalizeTime) < glanceMaxAimAge;
                if (
                    glanceEnable && !aimFresh && !brain->data->isClearance &&
                    brain->data->kickType == "shoot" &&
                    brain->msecsSince(_lastGlanceTime) > glanceCooldown
                ) {
                    newDecision = "glance";
                    _lastGlanceTime = now;
                    _glanceStartTime = now;
                } else if (threatLevel < threatThreshold) {
                    newDecision = "safe_shoot";
                    brain->data->kickSubType = 0;
                } else {
                    newDecision = "kick";
                    brain->data->kickSubType = 0;
    ...
    // 记录进入踢球类决策的时刻, 供上方的决策锁定 (P0-2) 使用
    bool isKickFamily = (newDecision == "kick" || newDecision == "safe_shoot" || newDecision == "cross");
    if (isKickFamily && !wasKickFamily) _kickCommitTime = now;
...
NodeStatus GoalieDecide::tick()
{
    ...
    // P1-3 fix: 原先此处有一个分支体被删空的 auto_visual_defend 分支 —
    // 当球在门将前方 0.5-2.0m 且前方无障碍时会产生空决策, 导致门将整 tick 无动作. 已整体移除.
    else if (ballRange > chaseRangeThreshold * (lastDecision == "chase" ? 0.9 : 1.0))
    {
        newDecision = "chase";
        color = 0x00FF00FF;
    ...
    {
        newDecision = "kick";
        // 门将树没有 CalcKickDir 节点, 这里把解围方向写入 data->kickDir, 供 Kick 节点沿 kickDir 穿球 (P0-3)
...
NodeStatus Kick::onStart()
{
    ...
    if (
        ...
        brain->client->setVelocity(-0.1, 0, 0, false, false, false);
        return NodeStatus::SUCCESS;
    }
    // 初始化 Node
    ...
    // 发布运动指令
    if (_state == "kick") {
        double angle = calcKickApproachAngle(brain); // P0-3: 沿规划的 kickDir 穿球, 而不是朝球直走
        ...
        // P0-2 fix: 原来传 -0.05 且 applyMinX=true, 会被放大成 -0.3m/s 连续后退 1 秒,
        // 直接把球退出 kick_range 导致踢球被放弃. 稳定阶段应原地站稳.
...
NodeStatus Kick::onRunning()
{
    ...
    if (
        ...
        brain->client->setVelocity(-0.1, 0, 0, false, false, false); // applyMinX 会把 -0.1 放大成 -0.3, 关闭
        return NodeStatus::SUCCESS;
    }
    if (_state == "stablize") {
        ...
        if (brain->msecsSince(_startTime) > msecs) {
            _state = "kick";
            _startTime = brain->get_clock()->now();
            double angle = calcKickApproachAngle(brain); // P0-3: 沿 kickDir 穿球
            double speed = getInput<double>("speed_limit").value();
            bool softKickoff;
            double softKickoffSpeed;
            brain->get_parameter("strategy.soft_kickoff", softKickoff);
...
void Kick::onHalted()
{
    // P2-3 fix: 原来是 _startTime -= Duration(100秒), 无意义(onStart 会重置)且单位错误. 改为停住.
    brain->client->setVelocity(0, 0, 0, false, false, false);
}
NodeStatus GlanceAtGoal::onStart()
{
    auto log = [=](string msg) {
        brain->log->setTimeNow();
        brain->log->log("debug/GlanceAtGoal", rerun::TextLog(msg));
    };
    auto fd = brain->config->fieldDimensions;
    Pose2D goal_r = brain->data->field2robot(Pose2D{fd.length / 2.0, 0.0, 0.0});
    _targetYaw = atan2(goal_r.y, goal_r.x);
    if (fabs(_targetYaw) > brain->config->headYawLimitLeft) {
        // 球门方向超出头部转动范围, 看也看不到, 直接结束避免浪费时间.
        // 注意也要停住: glance 决策会保持约 900ms, 不停住会沿用上一条速度指令漂移.
        brain->client->setVelocity(0, 0, 0, false, false, false);
        log(format("goal outside head range (yaw=%.2f), skip", _targetYaw));
        return NodeStatus::SUCCESS;
    }
    _startTime = brain->get_clock()->now();
    brain->client->setVelocity(0, 0, 0, false, false, false); // 观察期间站稳, 球位置由记忆维持
    brain->client->moveHead(0.25, _targetYaw);
    log(format("glancing at goal, yaw=%.2f", _targetYaw));
    return NodeStatus::RUNNING;
}
NodeStatus GlanceAtGoal::onRunning()
{
    int msecs = 600;
    brain->get_parameter("strategy.glance_at_goal.msecs", msecs);
    // 已经看到门柱(检测回调刷新了 lastGoalObservationTime) 或时间到, 就结束观察
    if (
        brain->msecsSince(brain->data->lastGoalObservationTime) < 200 ||
        brain->msecsSince(_startTime) > msecs
    ) {
        return NodeStatus::SUCCESS;
    }
    // 本节点在 CamFindAndTrackBall 之后 tick, 同一拍内后发的头部指令覆盖追球指令
    brain->client->setVelocity(0, 0, 0, false, false, false);
    brain->client->moveHead(0.25, _targetYaw);
    return NodeStatus::RUNNING;
}
...
NodeStatus RLVisionKick::onRunning()
{
    ...
    // Band-split stall 交接: 球已进入脚下区域 (handoff_range) 但 stall_ms 内基本没动,
    // 说明 RL 策略"只追不踢"(部分固件的已知问题), 退出交给确定性的脚本 Kick 兜底.
    double handoffRange = 0.85;
    int stallMs = 2500;
    brain->get_parameter("strategy.auto_visual_kick_handoff_range", handoffRange);
    brain->get_parameter("strategy.auto_visual_kick_stall_ms", stallMs);
    bool stallHandoff = false;
    if (brain->data->ballDetected && brain->data->ball.range < handoffRange) {
        double moved = _hasStallAnchor
            ? norm(brain->data->ball.posToField.x - _stallAnchorPos.x, brain->data->ball.posToField.y - _stallAnchorPos.y)
            : 0.0;
        if (!_hasStallAnchor || moved > 0.25) { // 球被踢动/明显移动, 重新锚定
            _hasStallAnchor = true;
            _stallAnchorPos = {brain->data->ball.posToField.x, brain->data->ball.posToField.y};
            _stallAnchorTime = brain->get_clock()->now();
        } else if (elapsedEnough && brain->msecsSince(_stallAnchorTime) > stallMs) {
            stallHandoff = true;
        }
    } else {
        _hasStallAnchor = false; // 球离开脚下区域, 停止 stall 计时
    }

    bool shouldExit = (((ballTooFar || costTooHigh) && elapsedEnough) || loseBallExit || ballOut || elapsedTimeout || stallHandoff);
    if (shouldExit) {
        string reasons = "";
        ...
        if (stallHandoff) appendReason(format("stall_handoff(range=%.2f,stall_ms=%d): RL 未完成踢球, 交给脚本 Kick", brain->data->ball.range, stallMs));
...
bool RLVisionKick::isMinIntervalSatisfied(double minIntervalMsec, const rclcpp::Time &now)
{
    // 原来是恒 true 的桩函数, 导致退出后立刻重入 (P1-4). 现按上次退出时间做真实冷却.
    if (now.get_clock_type() != _lastExitTime.get_clock_type()) return true; // 从未退出过
...
{
    // P2-3 fix: 原来把 msecs 当秒数做时间运算, 且 onStart 会重置 _startTime, 此处应为 no-op.
}
