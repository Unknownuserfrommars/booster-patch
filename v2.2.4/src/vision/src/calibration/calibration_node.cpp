...
void CalibrationNode::Init(const std::string cfg_path, bool is_offline, std::string calibration_mode) {
    ...
    // v1.6+ exposes a unified camera interface regardless of physical camera.
    // intrin 用 depth/camera_info: 与机器人上验证可用的 K1_5v5_Demo_v1.6_fixed 一致,
    // rgb/camera_info 在部分固件上不发布, 会导致 intrinsics 无法从话题刷新.
    std::string color_topic = "/boostercamera/head/rgb";
    std::string intrin_topic = "/boostercamera/head/depth/camera_info";
